...
context('API testing - test suit', () => {
    ...
    it('Get Api server status', () => { 
        
        cy.request({

            method: 'GET',
            url: baseUrl + apiServerStatus,
    ...
    it('Get all tools', () => { 
        ...
        cy.request({
            ...
            const body = response.body;
            const message = response.statusText;
            const jsonCount = response.body.length;

         
            expect(status).to.equal(200);
            expect(message).to.equal('OK');
            // expect(jsonCount).to.be.greaterThan(0);
            expect(jsonCount).to.equal(20);
    ...
    it('Get single tool', () => { 
            ...
            expect(status).to.equal(200);
            expect(message).to.equal('OK');
            expect(body).property('id').to.equal(4643);
            expect(body).property('category').to.equal('plumbing');
            expect(body).property('name').to.equal('PEX Clamp Tool');
            expect(body).property('manufacturer').to.equal('JWGJW');
            expect(body.price).to.equal(3.45);
            expect(body).property('current-stock').to.equal(14);
            expect(body).property('inStock').to.equal(true);
            expect(body).not.to.have.property('user-manual')   

          });

    })

    it('Get single tool with additional parameter', () => { 
        
        const userManual = ' ?user-manual=true'
               
        cy.request({

                method: 'GET',
                url: baseUrl + tools + toolId + userManual,
            ...
            expect(body).property('current-stock').to.equal(14);
            expect(body).property('inStock').to.equal(true);
            expect(body).property('user-manual').to.equal(userManualLink) 
    ...
    it('Register API client', () => { 
        ...
        const clientData = { 
            "clientName": "Postman",
            "clientEmail": "fizickolice" + randomNumber + "@example.com",
            }
       
        cy.request({

                method: 'POST',
                url: baseUrl + apiClient,
                body:  clientData
    ...
    it('Create order', () => { 
        ...
        cy.request({
            ...
            expect(status).to.eq(201);
            expect(message).to.eq('Created');
            newOrderId =  response.body.orderId  
            cy.log('OrderID: '+ newOrderId);                   
    ...
    it('Get all orders', () => { 
        ...
        cy.request({
            ...
            url: baseUrl + orders,
            headers: {
                      'Authorization': 'Bearer ' + authToken //Cypress.env('bearerToken')
                     },
    ...
    it('Get single order', () => { 
        ...
        cy.request({
            ...
            const message = response.statusText;
           // const jsonCount = response.body.length;
           // const length = response.body.jsonCount;

            response.body.length
         
            expect(status).to.equal(200);
            expect(message).to.equal('OK');
            // expect(jsonCount).to.be.greaterThan(1);
            // expect().to.equal(9);

            expect(body).property('id').to.equal(newOrderId);
            expect(body).property('toolId').to.equal(4643);
            expect(body).property('customerName').to.equal('Test customer');
            expect(body).to.have.property('createdBy')  
            expect(body).property('quantity').to.equal(1);
            expect(body).to.have.property('created')
            expect(body).to.have.property('timestamp')
            expect(body).to.have.property('processed')
    ...
    it('Validate updated order', () => { 
        ...
        cy.request({
            ...
            expect(body).property('id').to.equal(newOrderId);
            expect(body).property('customerName').to.equal('Test customer updated');
            expect(body).property('comment').to.equal('Updated comment');
